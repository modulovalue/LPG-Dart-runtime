 ...
 class BacktrackingParser extends Stacks
{
    ...
    //
     final int getToken(int i)
    {
        return tokens.get(locationStack[stateStackTop + (i - 1)]);
    }

     final int getCurrentRule()     { return currentAction; }
     final int getFirstToken()      { return tokStream.getFirstRealToken(getToken(1)); }
     final int getFirstToken(int i) { return tokStream.getFirstRealToken(getToken(i)); }
     final int getLastToken()       { return tokStream.getLastRealToken(lastToken); }
     final int getLastToken(int i)  { int l = (i >= prs.rhs(currentAction)
                                                        ? lastToken
     ...
     void reset(TokenStream tokStream)
    {
     ...
     void reset(Monitor monitor, TokenStream tokStream, ParseTable prs, RuleAction ra) throws BadParseSymFileException,
                                                                                                    NotBacktrackParseTableException
    {
     ...
     void reset(TokenStream tokStream, ParseTable prs, RuleAction ra) throws BadParseSymFileException,
                                                                                   NotBacktrackParseTableException
    {
     ...
     BacktrackingParser(TokenStream tokStream, ParseTable prs, RuleAction ra) throws BadParseSymFileException,
                                                                                           NotBacktrackParseTableException
    {
        reset(null, tokStream, prs, ra);
    }

     BacktrackingParser(Monitor monitor, TokenStream tokStream, ParseTable prs, RuleAction ra) throws BadParseSymFileException,
                                                                                                            NotBacktrackParseTableException
    {
     ...
     void reallocateOtherStacks(int start_token_index)
    {
        ...
        else if (this.actionStack.length < super.stateStack.length)
        {
            ...
            List.copyRange(this.actionStack, 0, this.actionStack = new int[super.stateStack.length], 0, old_length);
            List.copyRange(super.locationStack, 0, super.locationStack = new int[super.stateStack.length], 0, old_length);
            List.copyRange(super.parseStack, 0, super.parseStack = new Object[super.stateStack.length], 0, old_length);
        }
        return;
    }
    ...
    //
     Object fuzzyParse() throws BadParseException
    {
    ...
    //
     Object fuzzyParse(int max_error_count) throws BadParseException
    {
    ...
    // Always attempt to recover
    //
     Object fuzzyParseEntry(int marker_kind) throws BadParseException
    {
    ...
    //
    //
     Object fuzzyParseEntry(int marker_kind, int max_error_count) throws BadParseException
    {
        action.reset();
    ...
    //
     Object parse() throws BadParseException
    {
    ...
    // skip tokens.
    //
     Object parse(int max_error_count) throws BadParseException
    {
    ...
    // Parse without attempting any Error token recovery
    //
     Object parseEntry(int marker_kind) throws BadParseException
    {
        // without an argument parse() will ignore error productions
    ...
    // skip tokens.
    //
     Object parseEntry(int marker_kind, int max_error_count) throws BadParseException
    {
        action.reset();
        tokStream.reset(); // Position at first token.
        reallocateStateStack();
...
//System.err.println("Goto state " + prs.originalState(currentAction));
    ...
    //
     Object parseActions(int marker_kind) throws BadParseException
    {
        int ti = -1,
            curtok;
...
//System.err.println("reducing by rule " + currentAction);              
            ...
            else                   // a shift or shift-reduce action
            {
                if (tokStream.getKind(curtok) > NT_OFFSET) 
                {
                    ...
                    throw BadParseException(badtok.getErrorToken().getTokenIndex()); // parseStack[stateStackTop] = ra.prostheticAst[prs.getProsthesisIndex(tokStream.getKind(curtok))].create(tokStream.getIToken(curtok));
                }
                lastToken = curtok;
                curtok = tokens.get(++ti);
...
//else System.err.println("Shifting on token " + tokStream.getName(lastToken) + " to state " + prs.originalState(currentAction));              
    ...
    //
     int backtrackParse(IntSegmentedTuple action, int initial_token)
    {
        //
