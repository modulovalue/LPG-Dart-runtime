 ...
 class RecoveryParser extends DiagnoseParser implements ParseErrorCodes
{
    ...
    //
    //
     RecoveryParser(BacktrackingParser parser, IntSegmentedTuple action, IntTuple tokens, IPrsStream tokStream, ParseTable prs)
    {
        this(parser, null, action, tokens, tokStream, prs);
    }

     RecoveryParser(BacktrackingParser parser, Monitor monitor, IntSegmentedTuple action, IntTuple tokens, IPrsStream tokStream, ParseTable prs)
    {
        this(parser, monitor, action, tokens, tokStream, prs, 0, 0);
    }
    
     RecoveryParser(BacktrackingParser parser, IntSegmentedTuple action, IntTuple tokens, IPrsStream tokStream, ParseTable prs, int maxErrors, long maxTime)
    {
    ...
    //
     RecoveryParser(BacktrackingParser parser, Monitor monitor, IntSegmentedTuple action, IntTuple tokens, IPrsStream tokStream, ParseTable prs, int maxErrors, long maxTime)
    {
     ...
     void reallocateStacks()
    {
        ...
        else
        {
            ...
            List.copyRange(actionStack, 0, actionStack = new int[stateStack.length], 0, old_stack_length);
        }
     ...
     void reportError(int scope_index, int error_token)
    {
     ...
     int recover(int marker_token, int error_token) throws BadParseException
    {
        if (stateStack == null)
            reallocateStacks();
