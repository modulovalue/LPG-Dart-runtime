...
import 'TokenStream.dart';
 ...
 class PrimaryRepairInfo {
      ...
      PrimaryRepairInfo copy(){
         var c = PrimaryRepairInfo();
         c.distance = distance;
         c.misspellIndex = misspellIndex;
         c.code = code;
         c.bufferPosition = bufferPosition;
         c.symbol = symbol;
         return c;
     }
 }
...
class DiagnoseParser
{
     ...
     DiagnoseParser(TokenStream tokStream, ParseTable prs)
    {
     ...
     DiagnoseParser(Monitor monitor, TokenStream tokStream, ParseTable prs)
    {
        this(monitor, tokStream, prs, 0, 0);
    }
    
     DiagnoseParser(TokenStream tokStream, ParseTable prs, int maxErrors, long maxTime)
    {
    ...
    //
     DiagnoseParser(Monitor monitor, TokenStream tokStream, ParseTable prs, int maxErrors, long maxTime)
    {
     ...
     static final int BUFF_UBOUND  = 31,
                               BUFF_SIZE    = 32,
                               MAX_DISTANCE = 30,
                               MIN_DISTANCE = 3;
     ...
     int symbolIndex(int index)
    {
     ...
     String name(int index) { return prs.name(index); }
     ...
     ConfigurationStack main_configuration_stack;
     ...
     void reallocateStacks()
    {
        ...
        {
            List.copyRange(stateStack, 0, stateStack = new int[stack_length], 0, old_stack_length);
            List.copyRange(locationStack, 0, locationStack = new int[stack_length], 0, old_stack_length);

            List.copyRange(tempStack, 0, tempStack = new int[stack_length], 0, old_stack_length);
            List.copyRange(prevStack, 0, prevStack = new int[stack_length], 0, old_stack_length);
            List.copyRange(nextStack, 0, nextStack = new int[stack_length], 0, old_stack_length);
            List.copyRange(scopeIndex, 0, scopeIndex = new int[stack_length], 0, old_stack_length);
            List.copyRange(scopePosition, 0, scopePosition = new int[stack_length], 0, old_stack_length);
        }
        return;
    }
    ...
    //
     int parseCheck(int stack[], int stack_top, int first_symbol, int buffer_position)
    {
        int buffer_index,
            current_kind;
    ...
    //
     void checkPrimaryDistance(PrimaryRepairInfo repair, int stck[], int stack_top)
    {
        //
    ...
    //
     int getTermIndex(int stck[], int stack_top, int tok, int buffer_position)
    {
        //
     ...
     int misspell(int sym, int tok)
    {
     ...
     void scopeTrial(PrimaryRepairInfo repair, int stack[], int stack_top)
    {
     ...
     void scopeTrialCheck(PrimaryRepairInfo repair, int stack[], int stack_top, int indx)
    {
        ...
        if(statePoolTop >= statePool.length)
            List.copyRange(statePool, 0, statePool = new StateInfo[statePoolTop * 2], 0, statePoolTop);
    ...
    //
     bool secondaryCheck(int stack[], int stack_top, int buffer_position, int distance)
    {
    ...
    //
     void misplacementRecovery(SecondaryRepairInfo repair, int stack[], int stack_top, int last_index, bool stack_flag)
    {
    ...
    //
     void secondaryRecovery(SecondaryRepairInfo repair, int stack[], int stack_top, int last_index, bool stack_flag)
    {
        int previous_loc = buffer[2],
            stack_deletions = 0;
